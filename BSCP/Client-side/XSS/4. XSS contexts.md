...
4. コンテキストに合わせ改変したペイロードをエクスプロイトサーバにホストしてView Exploitで検証する[[#🚨：HTMLタグ内でのXSSペイロード注入時の注意点]]
```html
...
- 入力した内容がテンプレートリテラル([[JavaScriptとJSON#テンプレートリテラルとは]])に埋め込まれているケース
