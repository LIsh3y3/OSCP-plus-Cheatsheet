> [!NOTE]
この攻撃はWindowsが対象。Unixシステムの場合は偽サイトをホストして認証情報を入力させる手法が現実的。
...
- マクロの実行はデフォルトで無効にするよう設定された（Offic 2013～）：
...
ターゲットがMotWを解除してマクロを実行するには、以下の煩雑な手順が必要となり、攻撃者は新たなアプローチを考える必要が出た。
...
End Sub
```
- `CreateObject`：[用語](../用語.md#VBScript)
