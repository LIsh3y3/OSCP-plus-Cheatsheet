- 関連ノート：
	- [[⚡️Path traversal#SSH private key]]
	- [[🐉THC-Hydra]]
...
...
```
- →対策：`known_hosts`を削除する。([[#known_hostsとは]])
```zsh
rm /.ssh/known/hosts
```
...
- 初期侵入できていない段階で、パストラバーサルの脆弱性を用いてクライアントから秘密鍵を抽出し、その秘密鍵でSSH接続する：
	- [[⚡️Path traversal#SSH private keyの抽出]]
	- [[#SSH Private Keyの種類]]
- パスフレーズがある場合：ssh2john([[🐈‍⬛Password Crack - JtR・Hashcat#ハッシュ値の整形]])
...
- [[Tunneling Through Deep Packet Inspection#HTTP Tunneling w/ Chisel]]等の方法(ポートフォワーディング)で確立した*既存の*SOCKSプロキシ経由でSSH接続したいとき
	- [[Port Redirection & SSH Port Forwarding#SSH Local Dynamic Port Forwarding]]の`-D`のように、SSHがSOCKSプロキシを*新規で*用意して接続はできるが、既存のSOCKSプロキシ経由でのアクセスは不可
