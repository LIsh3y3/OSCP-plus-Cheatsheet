...
# 実行
./LinEnum.sh -r linenum.out -e /tmp/ -t
```
...
- 素早いチェック → `unix-privesc-check`
    軽量で「典型的な設定ミス」を見つけやすいく、古い環境でも動きやすい
- 全体像の把握 → LinEnum
    システム全体の情報を整理してくれるため出力をログとして持ち帰りたい時に便利
- 最新の攻撃視点での深掘り → LinPEAS
    実戦やCTFで一番よく使われる

| 優先度 | ツール名                                                                 | 特徴                                 | 出力内容                                                                                 | 長所                                                                       | 短所 / 注意点                                                  | 適用シーン                            |
| --- | -------------------------------------------------------------------- | ---------------------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------ | --------------------------------------------------------- | -------------------------------- |
| 1   | [unix-privesc-check](https://www.kali.org/tools/unix-privesc-check/) | シンプルなBashスクリプトで、古くからある             | 権限設定やファイルパーミッションの誤りなど、基本的な設定ミスをチェック                                                  | ・Kaliに**標準インストール済**<br>・軽量・依存関係が少ない<br>・「standard」と「detailed」モードを選べる     | ・開発更新が止まっている（v1.4が最終）<br>・検出範囲が限定的<br>・新しい攻撃手法やテクニックには未対応 | まずはシンプルに「明らかなミスコンフィグ」を確認したいとき    |
| 2   | [LinPEAS](https://github.com/peass-ng/PEASS-ng/tree/master/linPEAS)  | PEASの Linux版。積極的に開発更新されている。        | LinEnumで得られる情報 + カーネル脆弱性、権限昇格可能なバイナリ、sudo misconfig、capabilities、パスハイジャック等のより高度なチェック | ・アクティブに開発中で最新の手法に対応<br>・色分けで見やすい出力<br>・自動的に「 exploitable かも」と目立つ形で教えてくれる | ・出力が非常に膨大でノイズも多い<br>・実行が比較的重たい                            | 権限昇格候補を効率的に探したいとき（実戦ペンテストやCTF向き） |
| 3   | [LinEnum](https://github.com/rebootuser/LinEnum)                     | ・より包括的なBashスクリプト<br>・システム情報を広範囲に収集 | OS情報、ユーザ/グループ、SUID/SGIDファイル、cronジョブ、パーミッション不備、ネットワーク設定など                             | ・出力が詳細で、網羅的な情報収集が可能<br>・スクリプトが比較的わかりやすくカスタマイズしやすい                        | ・出力量が多すぎて整理が必要<br>・更新頻度はLinPEASに比べて少ない                    | 初期調査で「システムの全体像を把握」したいとき          |
