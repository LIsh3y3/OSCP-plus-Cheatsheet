...
- メモリバッファ（空き容量）を超えるデータを注入することで、予期せぬ処理(※)を引き起こす、メモリ破壊脆弱性の一種
	- (※)　DoS、RCEなど
- 理解に必要な知識：[コンピュータ・アセンブラ・バイナリの基礎知識](../Misc/コンピュータ・アセンブラ・バイナリの基礎知識.md)
...
- このとき、攻撃者が一定以上のAという文字をbufferに渡すと...

![](../画像ファイル/Pasted%20image%2020250604070135.png)

$$スタックに特定数のAを格納した図$$

- 💥Bufferが溢れて、Return address of main（リターンアドレス）が任意の値に上書きされてしまう
...
4. JMP ESP（または別のレジスタ）など、ペイロードを実行するための命令のアドレスでリターンアドレスを上書きする
	- [コンピュータ・アセンブラ・バイナリの基礎知識](../Misc/コンピュータ・アセンブラ・バイナリの基礎知識.md#ESPについて)
```c
...
- PythonよりもC言語を優先して確認
	- [Module 14：Fixing Exploits](#前提知識：CとPythonの違い)
...
- Pythonのような文字列の連結はできない↓（以下、Pythonなら可）
```python
string1 = "This is"
string2 = " a test"
...
6. ターゲットに初期侵害後か？
	...
	- [ ] //リバースエンジニアリングバイブルを見て、LotLなテクニックでどのDLLがセキュリティ保護機能が無効かを確認する方法を学習
	- No：ステップ7へ
...
| No  | 手法                                          | 説明                                                                                                                                          | 信頼性/再現性 | 備考                                                          |
| --- | ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ----------------------------------------------------------- |
| 1   | ローカルに再現した同一環境でモジュールの有無を探す                   | ・Immunity Debuggerなどでモジュールを探す<br>（[🧰Immunity debugger](../Tools/🧰Immunity%20debugger.md#モジュールの探し方)）                                                                        | 非常に高い   | 💡OSCP試験ではImmunity debuggerがすでに用意されている                      |
...
| 3   | ローカルに再現した同一環境で、ASLR等のセキュリティ保護機構が無効なモジュールを探す | [🧰Immunity debugger](../Tools/🧰Immunity%20debugger.md#セキュリティ機構が無効なモジュールの探し方)                                                                                               | 非常に高い   |                                                             |
...
- - ⚠️プログラミング言語の仕様に影響されず、バイナリとして扱わせるために、アドレスには`\x`とつけること
- ⚠️アドレスをリトルエンディアン or ビッグエンディアンで書くかに注意
...
1. 公開エクスプロイトからBad charsを特定する：[Module 14：Fixing Exploits](#Bad%20charsとは)
	...
	- ==アプリケーションや実行環境毎にBad charsは異なる場合がある==ので、検証が必要（しかし、OSCPの試験範囲にはエクスプロイトの開発は含まれていないので、他のエクスプロイトから引用する方法を優先する。）
...
- ==Exploit DBでVerifiedのエクスプロイトは、エラーやミスが0という意味ではない。==微調整してエクスプロイトに成功したことを検証した、という意味。
- 例えば、メモリ領域のバイト数がズレていることがある。

#### 動作検証

1. ターゲットを再現した環境で、Immunity debuggerを開く：[🧰Immunity debugger](../Tools/🧰Immunity%20debugger.md)
2. [Module 14：Fixing Exploits](#①ジャンプアドレスの特定・書き換え)で使用したアドレスを、Ctrl + Gで表示される検索ダイアログボックスに入力（CPUに関わらずビッグエンディアン）
